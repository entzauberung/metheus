import { BadgeCheck, ClipboardList, RefreshCw, SearchCheck, WandSparkles } from "lucide-react";
...
export function ExecutionPlanStep(props: Props) {
  ...
  const autopilotBanner = (props.autopilotActive && !autopilotRunning) ? (
  ...
  if (step === "PlanGeneration") return (
    <ConsoleStepShell icon={<ClipboardList />} title="生成执行计划" description={midStage?.title || "当前中阶段"}
      ...
      actions={autopilotActive ? undefined : <WorkflowActionBar><ActionButton icon={<WandSparkles size={16} />} loading={props.busy} loadingLabel="生成中" onClick={props.onGenerate}>生成执行计划</ActionButton></WorkflowActionBar>}>
      {autopilotBanner}
  ...
  return <ConsoleStepShell icon={<BadgeCheck />} title="批准执行计划" description={`${tasks.length} 个小阶段`}
    ...
    feedback={props.feedback} busy={props.busy}
    actions={(isApproved || props.autopilotActive) ? undefined : (<WorkflowActionBar>
