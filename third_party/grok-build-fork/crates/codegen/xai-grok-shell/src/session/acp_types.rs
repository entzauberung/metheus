...
pub struct StartupHints {
    ...
    /// Process-local Metheus embedding. Suppresses ordinary Grok host state
    /// such as the per-CWD prompt history without changing subagent semantics.
    #[serde(default)]
    pub metheus_embedded: bool,
    /// Frozen auth scheme supplied by the embedding host. Ordinary sessions
    /// continue resolving this from the Grok model catalog.
    #[serde(default)]
    pub metheus_auth_scheme: Option<String>,
    #[serde(default)]
