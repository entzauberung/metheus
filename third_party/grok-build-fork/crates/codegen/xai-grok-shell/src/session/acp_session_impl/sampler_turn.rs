...
impl SessionActor {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        if self.startup_hints.metheus_embedded {
            let auth_scheme = match self.startup_hints.metheus_auth_scheme.as_deref() {
                Some("x-api-key") => xai_grok_sampler::AuthScheme::XApiKey,
        ...
        let auth_scheme = match self.startup_hints.metheus_auth_scheme.as_deref() {
            Some("x-api-key") => xai_grok_sampler::AuthScheme::XApiKey,
            Some("bearer") => xai_grok_sampler::AuthScheme::Bearer,
            _ => model_facts.auth_scheme,
        };
    ...
    pub(crate) async fn handle_sampling_failure(
        ...
        let auth_recovery_eligible = !self.startup_hints.metheus_embedded
