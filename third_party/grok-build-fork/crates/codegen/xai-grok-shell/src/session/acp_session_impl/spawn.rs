...
pub(crate) async fn spawn_session_actor(
    ...
    metheus_file_policy: Option<
        xai_grok_tools::implementations::metheus_embedded::EmbeddedFilePolicy,
    >,
    metheus_embedded: bool,
) -> Result<
    (
        SessionHandle,
...
> {
    ...
    let (permissions, permission_events_rx, deny_read_globs) = if metheus_embedded {
    ...
    let context_window_override = if metheus_embedded {
        None
    } else {
    ...
    let mcp_strategy = if metheus_embedded {
        // There are no MCP servers in an embedded session. Blocking waits for
        // an initialization notification that can never arrive.
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let harness_metrics = if !metheus_embedded
    ...
    if let Some(policy) = metheus_file_policy {
        agent.tool_bridge().update_resource(policy).await;
    }
    agent
        .tool_bridge()
    ...
    if !metheus_embedded
        && let Some(section) = agent.agents_md_section()
    ...
    let resolved_workspace_root = if metheus_embedded {
    ...
    let mut effective_config = if metheus_embedded {
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        vcs_kind: if metheus_embedded {
            xai_grok_workspace::session::git::VcsKind::None
        } else {
        ...
        events: if metheus_embedded {
            crate::session::events::EventTracker::noop()
        } else {
            crate::session::events::EventTracker::new(&crate::session::persistence::session_dir(
                &session_info,
            ))
        },
...
> {
    ...
    let join_handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            let (initial_last_compaction, initial_prompt_texts) = if metheus_embedded {
                (None, Vec::new())
            } else {
                let session_dir = crate::session::persistence::session_dir(&session_info);
