...
impl MvpAgent {
    #[cfg(feature = "metheus-embedded")]
    pub(crate) fn new_metheus_embedded(
        ...
        cfg.metheus_embedded = true;
        cfg.default_yolo_mode = true;
        cfg.default_auto_mode = false;
        cfg.endpoints.alpha_test_key = None;
        ...
        sampling_config.origin_client = None;
        let agent = Self::with_models(gateway, &cfg, auth_manager, models_manager);
        *agent.sampling_config.borrow_mut() = sampling_config;
        agent.set_auth_method(acp::AuthMethodId::new("metheus.embedded"));
        Ok(agent)
    }

    #[cfg(feature = "metheus-embedded")]
    pub(crate) async fn spawn_metheus_embedded(
        &self,
        cwd: std::path::PathBuf,
        session_id: acp::SessionId,
        file_policy: xai_grok_tools::implementations::metheus_embedded::EmbeddedFilePolicy,
    ) -> Result<(), acp::Error> {
        let cwd = xai_grok_paths::AbsPathBuf::new(cwd)
            .map_err(|e| acp::Error::invalid_params().data(e.to_string()))?;
        let mut meta = acp::Meta::new();
        let auth_scheme = match self.sampling_config.borrow().auth_scheme {
            xai_grok_sampler::AuthScheme::XApiKey => "x-api-key",
            xai_grok_sampler::AuthScheme::Bearer => "bearer",
        };
        meta.insert(
            "startupHints".to_string(),
            serde_json::json!({
                "nonInteractive": true,
                "metheusEmbedded": true,
                "metheusAuthScheme": auth_scheme,
                "skipGitStatus": true,
            }),
        );
        let init = acp::InitializeRequest::new(acp::ProtocolVersion::V1)
            ...
            .meta(meta.clone());
        let model_id = acp::ModelId::new(self.sampling_config.borrow().model.clone());
        let spec = SessionSpawnOptions {
            ...
            client_fs_read: true,
            client_fs_write: true,
            preloaded_envrc: Some(std::collections::HashMap::new()),
            ...
            session_yolo_mode: true,
            session_auto_mode: false,
            prompt_display_cwd: None,
            metheus_embedded: true,
            metheus_file_policy: Some(file_policy),
        };
        self.spawn_and_register_session(&init, spec).await
    }

    #[cfg(feature = "metheus-embedded")]
    pub(crate) fn embedded_session_handle(
        &self,
        session_id: &acp::SessionId,
    ...
    #[cfg(feature = "metheus-embedded")]
    pub(crate) async fn shutdown_metheus_embedded(&self, session_id: &acp::SessionId) {
        ...
        if let Some(ops) = self.workspace_ops.borrow().as_ref() {
            ops.end_local_session(session_id.0.as_ref());
        }
    }
    ...
    pub(super) fn resolve_sampling_config_for_model(
        ...
        if self.cfg.borrow().metheus_embedded {
    ...
    pub fn with_models(
        ...
        let embedded = cfg.metheus_embedded;
        models_manager.set_gateway(gateway.clone());
        let sampling_config = if embedded {
            SamplingConfig::default()
        } else {
            models_manager.sampling_config()
        };
        if !embedded && !cfg.grok_com_config.api_key_auth_disabled() {
        ...
        let relay_config_enabled = !embedded
        ...
        let config_root = (!embedded)
            .then(|| crate::config::load_effective_config().ok())
            .flatten();
        ...
        let instance = Self {
            ...
            chat_modes: {
                ...
                if !embedded && crate::agent::chat_modes::process_chat_mode_enabled() {
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let metheus_embedded = self.cfg.borrow().metheus_embedded;
        ...
        let load_envrc = !metheus_embedded && self.cfg.borrow().session.load_envrc.unwrap_or(true);
        let startup_hints = init
            ...
            .unwrap_or_default();
        let hunk_plan = if metheus_embedded {
            HunkTrackingPlan { actor_mode: None }
        } else {
            plan_hunk_tracking(
            init
                .client_capabilities
        ...
        let loc_tracking_enabled = hunk_tracking_enabled && has_xai_auth
            && (self
                ...
                || std::env::var("GROK_LOC_TRACKING")
                    ...
                    .unwrap_or(false));
        let (feedback_resolved, mut feedback_flags) = {
            let cfg = self.cfg.borrow();
        ...
        let project_env_trusted = !metheus_embedded
            && folder_trust::project_scope_allowed(cwd.as_path());
        let mut session_env = if metheus_embedded {
            std::collections::HashMap::new()
        } else {
            xai_grok_workspace::permission::claude_settings::load_claude_env_with_project(
                cwd.as_path(),
                project_env_trusted,
            )
        };
        ...
        if !metheus_embedded {
            ...
            tool_ctx.is_turn_active = Some(self.subagent_coordinator.borrow().turn_active_flag());
        ...
        let support_permission =
            !metheus_embedded && self.cfg.borrow().features.support_permission;
        let telemetry_enabled = !metheus_embedded && self.product_analytics_enabled();
        let origin_client = self.origin_client_info_from_meta(init.meta.as_ref());
        let sampling_config = if metheus_embedded {
        ...
        let auto_update = if metheus_embedded {
            None
        } else {
            self.cfg.borrow().cli.auto_update
        };
        ...
        let (
            ...
            (None, None, None, None)
        } else if let Some((url, token, alpha, deploy)) = self.feedback_credentials() {
        ...
        let skills = if metheus_embedded {
            Default::default()
        } else {
            self.cfg.borrow().skills.clone()
        };
        let compat = if metheus_embedded {
            Default::default()
        } else {
            self.cfg.borrow().compat_resolved
        };
        let acp_agent_profile = (!metheus_embedded)
            .then(|| parse_agent_profile_from_meta(session_meta))
            .flatten();
        let session_default_agent_profile = acp_agent_profile
            .as_ref()
            .map(|d| d.name.clone());
        let mut agent_definition = if metheus_embedded {
            embedded_agent_definition()
        } else {
            let cfg = self.cfg.borrow();
        ...
        if !metheus_embedded && let Some(template) = inherited_harness_template(
        ...
        let (session_model_id, sampling_config) = if metheus_embedded {
            (session_model_id, sampling_config)
        } else {
            self.apply_agent_model_override(
                pinned_model.as_ref(),
                session_model_id,
                sampling_config,
                origin_client.clone(),
        ...
        let lsp_tools_enabled = !metheus_embedded && self.cfg.borrow().resolve_lsp_tools().value;
        ...
        let inference_idle_timeout_secs = if metheus_embedded {
            300
        } else {
            let models = self.models_manager.models();
            let cfg = self.cfg.borrow();
        ...
        let video_gen_config = if metheus_embedded { Default::default() } else { self.prepare_video_gen_config() };
        let app_builder_deployer_config = if metheus_embedded { Default::default() } else { self.prepare_app_builder_deployer_config() };
        let web_fetch_config = if metheus_embedded { xai_grok_tools::implementations::grok_build::web_fetch::WebFetchConfig::Disabled } else { self.prepare_web_fetch_config() };
        let write_file_enabled = !metheus_embedded && self.cfg.borrow().resolve_write_file().value;
        let goal_enabled = !metheus_embedded && self.cfg.borrow().resolve_goal().value;
        let subagents_enabled = !metheus_embedded && self.cfg.borrow().subagents_enabled;
        let ask_user_question_enabled = crate::upload::turn::parse_ask_user_question_from_meta(
                session_meta,
            )
            .unwrap_or_else(|| !metheus_embedded && self.cfg.borrow().resolve_ask_user_question().value);
        let client_hooks = if metheus_embedded { Default::default() } else { crate::extensions::hooks::parse_client_hooks(session_meta) };
        let disable_web_search = metheus_embedded || self.cfg.borrow().disable_web_search;
        let todo_gate = !metheus_embedded && self.cfg.borrow().todo_gate;
        let remote_settings_for_spawn = if metheus_embedded { None } else { self.cfg.borrow().remote_settings.clone() };
        let laziness_debug_log_for_spawn = if metheus_embedded { None } else { self.cfg.borrow().laziness_debug_log.clone() };
        let respect_gitignore = !metheus_embedded && self.cfg.borrow().respect_gitignore;
        let path_not_found_hints = !metheus_embedded && self.cfg.borrow().path_not_found_hints;
        let subagent_toggle = if metheus_embedded {
            std::collections::HashMap::new()
        } else {
            self.cfg.borrow().subagent_toggle.clone()
        };
        let handle_display_cwd = prompt_display_cwd.clone();
        let auth_manager = if metheus_embedded {
            None
        } else {
            Some(self.auth_manager.clone())
        };
        let bash_params_json = if metheus_embedded {
            serde_json::Map::new()
        } else {
            let cfg = self.cfg.borrow();
        ...
        let ask_user_question_params_json = if metheus_embedded {
            None
        } else {
            let cfg = self.cfg.borrow();
        ...
        let backend_tools_enabled = if metheus_embedded {
            false
        } else {
            let cfg = self.cfg.borrow();
            cfg.resolve_backend_tools().value
        };
        let managed_mcp_proxy_url = if metheus_embedded {
            String::new()
        } else {
            self.cfg.borrow().endpoints.proxy_url()
        };
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = xai_chat_state::Credentials {
                ...
                alpha_test_key: if metheus_embedded {
                    None
                } else {
                    self.alpha_test_key()
                },
            ...
            let acp_mcp_servers = if metheus_embedded {
                Vec::new()
            } else {
...
/// The embedded host owns the policy boundary. Keep this definition in the
/// upstream runtime so its normal AgentBuilder and SessionActor turn loop are
/// still used, while no shell-capable or discoverable tools can be injected.
fn embedded_agent_definition() -> xai_grok_agent::AgentDefinition {
    use xai_grok_agent::config::{AgentDefinition, McpInheritance, PermissionMode};
    ...
    definition.permission_mode = PermissionMode::BypassPermissions;
    ...
    definition.session_tools_allowlist = Some(vec![
        "GrokBuild:read_file".to_string(),
        "GrokBuild:search_replace".to_string(),
        "GrokBuild:list_dir".to_string(),
        "GrokBuild:grep".to_string(),
    ]);
    definition
}

#[cfg(test)]
mod metheus_embedded_definition_tests {
    use super::*;

    #[test]
    fn embedded_agent_definition_has_exact_file_tool_boundary() {
        let definition = embedded_agent_definition();
        let tool_ids = definition
            .tool_config
            .tools
            .iter()
        ...
        let expected_allowlist = tool_ids
            .iter()
            .map(|id| (*id).to_string())
            .collect::<Vec<_>>();
        assert_eq!(
            definition.session_tools_allowlist.as_deref(),
            Some(expected_allowlist.as_slice())
        );
        ...
        assert!(definition.skills.is_empty());
