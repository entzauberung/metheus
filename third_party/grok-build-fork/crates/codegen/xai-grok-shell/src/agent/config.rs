...
pub struct Config {
    /// Process-local embedding mode used by audited hosts. This is never
    /// deserialized from Grok configuration and must be enabled explicitly by
    /// the embedding constructor.
    #[serde(skip)]
    pub(crate) metheus_embedded: bool,
