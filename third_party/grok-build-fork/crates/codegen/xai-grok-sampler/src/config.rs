...
impl fmt::Debug for SamplerConfig {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("extra_headers_configured", &self.extra_headers.len())
...
mod tests {
    ...
    fn debug_redacts_api_key_and_extra_header_values() {
        let mut config = SamplerConfig {
            api_key: Some("metheus-secret-sentinel".to_string()),
            ..Default::default()
        };
        config.extra_headers.insert(
            "x-private-token".to_string(),
            "metheus-header-sentinel".to_string(),
        );

        let debug = format!("{config:?}");
        assert!(!debug.contains("metheus-secret-sentinel"));
        assert!(!debug.contains("metheus-header-sentinel"));
        assert!(debug.contains("api_key_configured: true"));
        assert!(debug.contains("extra_headers_configured: 1"));
    }
