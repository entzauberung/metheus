...
impl xai_tool_runtime::Tool for ListDirTool {
    ...
    async fn run(
        ...
        let embedded_policy = resources
            .lock()
            .await
            .get::<crate::implementations::metheus_embedded::EmbeddedFilePolicy>()
            .cloned();
        if let Some(policy) = embedded_policy {
            return Ok(match policy.list_dir(&input.target_directory) {
                Ok((absolute_root_path, content)) => {
                    ListDirOutput::Content(crate::types::output::ListDirContent {
                        content,
                        absolute_root_path,
                    })
                }
                Err(message) => ListDirOutput::PermissionDenied(message),
            });
        }
