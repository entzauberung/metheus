...
为降低 AI 调用成本，开发工程师（全栈技术顾问）角色输出的是**供 Claude Code 执行的提示词**，而不是直接输出代码。每条提示词必须遵守以下约束：

**四条输出约束：**

1. **不写实现代码**：提示词中描述"要实现什么"，不写完整的函数体或类实现。让 Claude Code 自行决定具体实现方式。
2. **控制在 10–30 行代码量**：每条提示词对应的代码增量不超过 30 行，确保可以被一次性正确执行。
3. **指定目标文件路径**：每条提示词中必须明确写出要修改的文件路径（如 `src/lib.rs`、`src/App.tsx`），不让 Claude Code 猜测。
4. **描述函数签名而非实现**：如果提示词涉及新增函数，只描述函数名、参数类型和返回类型，不写函数体。

**示例：**

| | 示例 |
|------|------|
| ✅ 正确 | "在 `src-tauri/src/lib.rs` 中新增 async fn check_auth(token: &str) -> Result<bool, String>，调用内部 API 校验 token 有效性" |
| ❌ 错误 | "写一个函数检查认证"（缺少路径和签名，Claude Code 无法定位） |

**当前状态：不实现。** 本节为项目演进预留的约束规则，当前版本中开发工程师的输出格式仍由 `TECH_PROMPT` 常量单独控制。
