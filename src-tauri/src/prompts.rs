...
你是当前编码任务的恢复规划器。常规增量修复已经耗尽，请根据原任务契约和失败历史，生成一次可从任务开始前 Git 基线完整重执行的提示。\n\
硬性约束：\n\
1. 验收标准是最高优先级；原执行提示只能补充，不能覆盖验收标准。\n\
2. 必须覆盖所有验收项，不得只描述最后一次失败的剩余差异。\n\
3. 不得修改目标、验收标准、允许修改/新建文件、停止规则或任务边界。\n\
4. 不得增加 UI、重构或其他未要求工作。\n\
5. 精确函数名、字段名、API 名和行为必须逐字遵循验收标准。\n\
只输出 JSON 对象：{\"execution_prompt\":\"自包含的完整执行提示\",\"covered_criteria\":[1,2],\"rationale\":\"如何避免历次失败的简短说明\"}。covered_criteria 必须使用从 1 开始的编号并完整覆盖所有验收项。";
#[allow(dead_code)]
...
- 每个小阶段对象包含以下字段：\
  ...
  - prompt：字符串，可执行的编码任务提示词（详细描述要完成的功能目标）。\
\
**内容约束：**\
- 不得在提示词中包含完整的代码块。\
- 提示词应描述「做什么」（功能目标），而不是「写什么」（具体代码实现）。\
...
4. 只注入该任务需要的精确上下文。\
5. execution_prompt 必须逐字复述 acceptance_criteria 中出现的函数名、字段名、API 名和关键行为，不得用近义命名替代。\
6. execution_prompt 与 acceptance_criteria 存在歧义或冲突时不得生成计划，必须通过输出契约校验失败暴露问题。\
\
**每个小阶段必须包含：**\
...
- execution_prompt：字符串，面向编码执行引擎的最终执行提示（描述做什么，不是写什么代码）。\
\
**输出格式：**\
输出严格的 JSON 数组，以 [ 开头，以 ] 结尾。不要 markdown 标记，不要解释文字。";
