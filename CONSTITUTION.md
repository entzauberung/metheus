...
- 新建 No Project 与 Half Project 默认进入 `Shadow`，缺少控制字段的旧项目仍按 `Legacy` 加载，磁盘已有模式保持不变。Shadow 只返回旧策略命令，并持久化可比较一致、可比较差异、不可比较和最近一次对照；不会派发新控制动作，也不会自动升级为 `SerialTakeover`。
...
- `Legacy` 与 `Shadow` 回退路径继续可用，旧项目缺少控制字段时仍默认 `Legacy`；`SerialTakeover` 没有被设置为默认模式。
