...
> 最后同步：2026-07-21。本文描述 Metheus 仓库当前实现；`src-tauri/CONSTITUTION.md` 是测试/目标项目生成出的项目宪法，不是本仓库的开发约束来源。
...
- **错误恢复状态**：`RecoveryState`，记录错误类型、恢复阶段、尝试次数、错误签名、执行会话和诊断证据
...
│  └────────────┘ └────────────┘ └────────────┘ └───────────┘ │
│  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌───────────┐ │
│  │ executor   │ │ test_runner│ │plan_contract│ │constitution│ │
│  │ Claude CLI │ │ 测试与审查  │ │ 精确范围契约 │ │ 滚动宪法   │ │
│  └────────────┘ └────────────┘ └────────────┘ └───────────┘ │
│  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌───────────┐ │
...
recovery ──→ pipeline, executor, test_runner, plan_contract, project, lib(基础fn)
...
> **说明**：以下模块清单反映 2026-07-21 的当前代码状态；历史兼容入口仍存在，但不得参与新恢复链路的业务裁决。
...
| **关键类型** | `Project`, `WorkflowState`, `AutopilotState`, `RecoveryState`, `RecoveryErrorKind`, `RecoveryPhase`, `ExecutionSession`, `Milestone`, `MidStage`, `Subtask`, `ExecutionResult`, `TestResult`, `AutomatedTestStatus`, `VerificationKind`, `HumanVerification`, `PauseContext`, `ManagedFlowState` |
...
| 函数 | `execute_subtask_inner`（Claude CLI 子进程、流式输出、暂停/超时和 `execution_id` 隔离） |
...
| `project_ops.rs` | `get_project`, `initialize_project_entry`, `validate_project_path`, `get_project_files`, `persist_project`(兼容入口), `approve_mid_stage`, `reject_mid_stage` |
...
| `workflow.rs` | 工作流迁移与转换、ThreeChecks 入口、讨论恢复、Managed Flow、autopilot 启停/暂停/恢复/下一步路由 |
...
| `src/ProjectEntry.tsx` | Before 入口页面：No Project 和 Half Project 选择、路径校验 |
...
| `RecoveryState` | 当前小阶段的持久化错误恢复状态 |
| `RecoveryErrorKind` | Workspace/Transient/Execution/Scope/Test/Review/Unavailable/Conflict/Human 分类 |
| `RecoveryPhase` | Diagnosing / Repairing / Retesting / Recovered / WaitingHuman |
...
| **Claude Code 执行失败** | 保存执行证据；手动模式提供基线恢复，autopilot 进入 `ExecutionError` 恢复分支并先恢复执行基线 |
| **Claude Code 子进程卡死** | 600s 超时强制 kill，按执行错误收尾，不在未知工作区上继续 |
...
恢复诊断只允许包含当前目标、验收标准、`allowed_file_paths`、`new_file_paths`、受限 diff、测试命令/退出码/压缩输出、审查问题、上次修复摘要和 Git 基线。禁止重新规划整个项目或注入全量历史。
