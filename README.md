...
<p align="center"><em>面向真实软件生产的 AI 任务编译系统</em></p>

<p align="center">Metheus 将原本需要大模型整体处理的复杂需求，编译成小模型可以独立完成的执行单元，以高并发、高吞吐的并行执行降低 token 成本。</p>

<p align="center"><strong>目标是更少token，更少花费，更快的速度，让更多人能把想法变成软件。</strong> v0.0.4 正在开发中。</p>
...
| 让同一个强模型包办所有步骤 | 高层决策；受约束、可验证的执行，会全部编译成小模型的执行单元，没有模型强弱的分化 |
...
v0.0.4 第一阶段已完成的控制基础中，`SerialTakeover` 是新项目的正式默认串行模式，负责任务执行阶段的派发与恢复决策；`Shadow` 只做对照审计，`Legacy` 只用于兼容和显式人工回退。旧项目保留磁盘中已有模式，不会自动迁移。这个基础不表示 v0.0.4 整体或真正并行执行已经完成。
...
```
⚠️ 轻度机器学习以及hermes记忆系统成本仍然测试起来会显著增加token，但是会让出错率显著降低
⚠️ 其他多模型的测试没有完全覆盖
